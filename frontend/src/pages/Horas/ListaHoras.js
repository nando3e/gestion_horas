...
const ListaHoras = () => {
  ...
  const handleOpenEditModal = async (hora) => {
    ...
    try {
      ...
      if (hora.id_obra) {
        ...
        setPartidas(partidasData.filter(p => p.id_obra === hora.id_obra && (!p.acabada || p.id_partida === hora.id_partida)));
      } else {
        setPartidas([]);
      }
    } catch (error) {
  ...
  // Cambiar la obra desde el modal (solo admin): recarga partidas y obliga a elegir una de la nueva obra
  const handleObraModalChange = async (nuevaObraId) => {
    setObraSeleccionada(nuevaObraId);
    setNombreObra(obras.find(o => o.id_obra === parseInt(nuevaObraId))?.nombre_obra || '');
    setPartidaSeleccionada('');
    setModalError('');
    try {
      const partidasData = await partidasService.getPartidasPorObra(nuevaObraId);
      setPartidas(partidasData.filter(p => p.id_obra === parseInt(nuevaObraId) && !p.acabada));
    } catch (error) {
      console.error("Error al cargar partidas:", error);
      setModalError("No se pudieron cargar las partidas de la obra seleccionada.");
      setPartidas([]);
    }
  };
        ...
        >
          ...
          <DialogContent>
            ...
            <Grid container spacing={2} sx={{ mt: 1 }}>
                ...
                {puedeCambiarAdjudicacion ? (
                  <FormControl fullWidth required>
                    ...
                    <Select
                      ...
                      label="Obra"
                      onChange={(e) => handleObraModalChange(e.target.value)}
                    >
                      {obras.map(o => (
                        <MenuItem key={o.id_obra} value={o.id_obra.toString()}>
                          {o.nombre_obra}
                        </MenuItem>
                      ))}
                    </Select>
              ...
              {/* Partida: editable solo para admin */}
              <Grid item xs={12} md={6}>
                <FormControl fullWidth required>
                  <InputLabel>Partida</InputLabel>
                  <Select
                    ...
                    disabled={!puedeCambiarAdjudicacion}
                  >
